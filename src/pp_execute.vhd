...
entity pp_execute is
	...
	port(
		...
		rob_op_num_in   : in  integer;
		exe_op_num_out  : out integer;
		...
		count_instruction_csr_in  : in  std_logic;
		count_instruction_csr_out : out std_logic;
...
architecture behaviour of pp_execute is 
	...
	signal alu_x, alu_y, alu_result, prev_alu_result : std_logic_vector(31 downto 0);
...
begin
	...
	rs1_data <= rs1_data when stall = '1' or prev_stall = '1' else rs1_data_in;
	rs2_data <= rs2_data when stall = '1' or prev_stall = '1' else rs2_data_in;
	...
	prev_alu_result <= alu_result when ((stall = '0' and prev_stall = '0') or (stall='1' and prev_stall='0')) else prev_alu_result;
	...
	begin
		if rising_edge(clk) then
			if reset = '1' or flush = '1' then
				...
				count_instruction_csr_out <= '0';
				exe_op_num_out <= LENGTH_MAIN; 
			elsif stall = '1' then
				csr_write <= CSR_WRITE_NONE;
			elsif stall = '0' then
				pc <= pc_in;
				count_instruction_out <= count_instruction_in;
				count_instruction_csr_out <= count_instruction_csr_in;
	...
	rs1_csr_forward: process(csr_write, rs1_addr, rs1_data, 
		mem_csr_write, mem_rd_addr, mem_count_instr_csr, mem_rd_value,
		wb_csr_write, wb_rd_addr, wb_count_instr_csr, wb_rd_value)
	begin
		if csr_write /= CSR_WRITE_NONE and mem_csr_write /= CSR_WRITE_NONE and mem_rd_addr = rs1_addr and mem_count_instr_csr = '1' then --and mem_csr_addr /= b"00000"  then
			rs1_forwarded <= mem_rd_value;
		elsif csr_write /= CSR_WRITE_NONE and wb_csr_write /= CSR_WRITE_NONE and wb_rd_addr = rs1_addr and wb_count_instr_csr = '1' then --and wb_rd_addr /= b"00000"  then
			rs1_forwarded <= wb_rd_value;
		else
			rs1_forwarded <= rs1_data;
		end if;
	end process rs1_csr_forward;
