...
run-tests-vcd: potato.prj compile-tests
	for test in $(RISCV_TESTS) $(LOCAL_TESTS); do \
		...
		DMEM_FILENAME="empty_dmem.hex"; \
		test -f tests-build/$$test-dmem.hex && DMEM_FILENAME="tests-build/$$test-dmem.hex"; \
		xelab tb_processor -debug typical -generic_top "IMEM_FILENAME=tests-build/$$test-imem.hex" -generic_top "DMEM_FILENAME=$$DMEM_FILENAME" -prj potato.prj > /dev/null; \
...
run-soc-tests-vcd: potato.prj compile-tests
	for test in $(RISCV_TESTS) $(LOCAL_TESTS); do \
		...
		DMEM_FILENAME="empty_dmem.hex"; \
		test -f tests-build/$$test-dmem.hex && DMEM_FILENAME="tests-build/$$test-dmem.hex"; \
		xelab tb_soc -debug typical -generic_top "IMEM_FILENAME=tests-build/$$test-imem.hex" -generic_top "DMEM_FILENAME=$$DMEM_FILENAME" -prj potato.prj > /dev/null; \
		echo "open_vcd tests-build/$$test.vcd; log_vcd [get_objects -r *]; run all; close_vcd; quit" > tests-build/xsim_wave.tcl; \
		xsim tb_soc -tclbatch tests-build/xsim_wave.tcl > tests-build/$$test.results-soc; \
		cat tests-build/$$test.results-soc | awk '/Note:/ {print}' | sed 's/Note://' | awk '/Success|Failure/ {print}'; \
	done
